...
bmore_tracts <- get_acs(geography = "tract", 
                           ...
                           county = c(510,5,3,27),
                           survey = "acs5",
                           state = c("Maryland"), # 24 is the FIPS code for MD
                           geometry = TRUE, 
                           output = "wide") %>%
...
```{r writefiles, eval = FALSE}
st_write(bmore_tracts, "../data/bmore_tracts.geojson", delete_dsn = TRUE)
st_write(md_counties, "../data/md_counties.geojson", delete_dsn = TRUE)
st_write(md_state, "../data/md_state.geojson", delete_dsn = TRUE)
```
