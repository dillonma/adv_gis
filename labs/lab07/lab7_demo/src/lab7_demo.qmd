...
This chunk downloads the data and uses `dplyer` pipes to send the result to `st_transform` and then to `erase_water`. The result is an `sf` object in an Albers projection for North America.
...
This chunk makes several calculations ...

### Race and Ethnicity Groupings

First, we problematically, but conveniently, reduces the race and ethnicity into 5 categories:

-   Non-Hispanic white as `pop_nhwhiteE` (from the Census)
-   Non-Hispanic Black as `pop_nhblackE` (from the Census)
-   Hisp/Latx as `pop_hispltxE` (from the Census)
-   Non-Hispanic Asian as `pop_nhasianXE` (computed below)
-   Non-Hispanic Multi/Other as `pop_nhotherXE` (computed below)

### Housing Vacancy

Second we calculate the total vacancy by subtracting the total housing units by the total occupied housing units. Note that we compute this as `hu_totalvacXE`.

### Percent White, Non-Hispanic

Lastly we calculate the percent of non-Hispanic white people in a census tract by dividing the `pop_nhwhiteE` (number of non-Hispanic white) from `popE` (the total population).
...
Let's also get a backdrop for Maryland, counties and the state. This is so just in case you need them when you make maps for QGIS.
